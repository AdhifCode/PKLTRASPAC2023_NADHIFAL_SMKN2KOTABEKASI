...
page = requests.get(
    "https://m.brilio.net/search-result/" + search + "/"
)
...
def crawling_data(halaman):
    ...
    angka = str(halaman)
    page = requests.get(
        "https://m.brilio.net/search-result/" + search + "/index" + page + ".html"
    )
    soup = BeautifulSoup(page.text, "html.parser")
    ...
    query = (
        "INSERT INTO scraping_brilio"
        + " (judul,konten,dibuat_pada) VALUES (%s,%s,%s)"
    )
    insert_data(query, array_data)

def insert_data(query, array_data, nama_db="web_scraping"):
    try:
